...
Field::Field(const Field& field) :
	m_size(field.m_size), m_isGot(false), m_cells(0), m_ships(0), m_aliveShipCount(0), m_selectedShip()
{
	for (std::shared_ptr<Cell> c : field.m_cells)
		m_cells.push_back(std::make_shared<Cell>(Cell{ *c }));
	for (std::shared_ptr<Ship> s : field.m_ships)
		m_ships.push_back(std::make_shared<Ship>(Ship{ *s }));
	if (field.m_selectedShip != nullptr)
	{
		m_selectedShip = std::make_shared<std::vector<Ship>::const_iterator>(find(field.m_ships.begin(), field.m_ships.end(), field.m_selectedShip));
	}
}
int Field::GetSize()
{
